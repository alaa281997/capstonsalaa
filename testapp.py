...
class CastingTest(unittest.TestCase):
    ...
    def test_get_actorid(self):
        response = self.client().get(
            '/actors/1', headers={'Authorization':'Bearer '+ casting_assistant}
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code,200)
        self.assertEqual(data['success'],True)
    ...
    def test_404_get_actorid(self):
        response = self.client().get(
            '/actors/100',headers={"Authorization":"Bearer" + casting_assistant}
    ...
    def test_400_postactor(self):
        response = self.client().post(
            '/actors',json={},headers={'Authorization':f'Bearer{executive_producer}'}
        )
        data = json.loads(response.data)
    ...
    def test_delete_actor(self):
        response = self.client().delete(
            '/actor/2',headers={'Authorization':f'Bearer{executive_producer}'}
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code,401)
        self.assertEqual(data['code'],'unauthorized')
    ...
    def test_404_delActor(self):
        response = self.client().delete(
            '/actors/1234',headers={'Authorization':f'Bearer{executive_producer}'}
        )
        data = json.loads(response.data)
    ...
    def test_get_movie(self):
        response = self.client().get(
            '/movies/1', headers={'Authorization':f'Bearer{casting_assistant}'}
        )
        data = json.loads(response.data)
    ...
    def test_404_getmovies(self):
        response = self.client().get(
            '/movies/100',headers ={'Authorization':f'Bearer {casting_assistant}'}
        )
        data = json.loads(response.data)
    ...
    def test_400_postmovie(self):
        response = self.client().post(
            '/movies',json={},headers={'Authorization':f'Bearer{executive_producer}'}
        )
        data = json.loads(response.data)
        self.assertEqual(response.data)
        self.assertEqual(data['success'],False)
    ...
    def test_401_postmovie_unauthorized(self):
        response = self.client().post(
            '/movies',json=self.test_get_movie,headers={'Authorization':f'Bearer {casting_assistant}'}
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code,401)
        self.assertEqual(data['code'],'unauthorized')
    ...
    def test_400_patchmovie(self):
        response = self.client().patch(
            '/movies/1',json={},headers = {'Authorization':f'Bearer {executive_producer}'}
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code,400)
        self.assertEqual(data['success'],False)
