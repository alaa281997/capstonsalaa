...
class CastingTest(unittest.TestCase):
    ...
    def test_create_new_actor(self):
        ...
        response = self.client().get(
            '/actors',headers={ "Authorization": (casting_assistant ) })
    ...
    def test_post_actor(self):
        response = self.client().post(
            '/actors', json={'name':'miley','age':20,'gender':'female' },
                      headers={ "Authorization": (executive_producer) }
        )
        data = json.loads(response.data)
    ...
    def test_400_postactor(self):
        response = self.client().post(
            '/actors',json={},headers={ "Authorization": (executive_producer ) }
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code,401)
    ...
    def test_delete_actor(self):
        response = self.client().delete(
            '/actors/2',headers={ "Authorization": (executive_producer ) }
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code,401)
        self.assertEqual(data['success'], True)
    ...
    def test_404_delActor(self):
        response = self.client().delete(
            '/actors/1234',headers={ "Authorization": (executive_producer ) }
        )
        data = json.loads(response.data)
    ...
    def test_get_movies(self):
        response = self.client().get(
            '/movies',headers={ "Authorization": (casting_assistant ) }
        )
        data = json.loads(response.data)
    ...
    def test_400_postmovie(self):
        response = self.client().post(
            '/movies',json={},headers={ "Authorization": (executive_producer ) }
        )
        data = json.loads(response.data)
        self.assertEqual(response.data,400)
        self.assertEqual(data['success'],False)
        self.assertTrue(data['error'],400)
        self.assertEqual(data['message'], 'bad request')
    ...
    def test_401_postmovie_unauthorized(self):
        response = self.client().post(
            '/movies',json={'title':'stud','release_date':'1-1-2021'},headers={ "Authorization": (casting_assistant ) }
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code,401)
        self.assertEqual(data['message'],'unauthorized')
    ...
    def test_401_patchmovie(self):
        response = self.client().patch(
            '/movies/1',json={},headers={ "Authorization": (executive_producer ) }
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code,401)
        self.assertEqual(data['success'],False)
    ...
    def test_delete_movie(self):
        response = self.client().delete(
            '/movies/2',headers={ "Authorization": (executive_producer ) }
    ...
    def test_401_deletemovie(self):
        response = self.client().delete(
            '/movies/2',headers={ "Authorization": (casting_assistant ) }
        )
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 401)
        self.assertEqual(data['message'], 'unauthorized')
    ...
    def test_404_deletemovie(self):
        response = self.client().delete(
            '/movies/1234',headers={ "Authorization": (executive_producer ) }
        )
        data = json.loads(response.data)
